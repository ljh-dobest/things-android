...
import com.aijia.ipcameraproject.BridgeService.PlayInterface;
import com.aijia.ipcameraproject.adapter.ViewPagerAdapter;
import com.aijia.ipcameraproject.bean.AttentionEvent;
import com.aijia.ipcameraproject.bean.LoginResponse;
import com.aijia.ipcameraproject.bean.MessageEvent;
import com.aijia.ipcameraproject.service.BrainLinkService;
import com.aijia.ipcameraproject.utils.AudioPlayer;
import com.aijia.ipcameraproject.utils.ContentCommon;
import com.aijia.ipcameraproject.utils.CustomAudioRecorder;
import com.aijia.ipcameraproject.utils.CustomBuffer;
import com.aijia.ipcameraproject.utils.CustomBufferData;
import com.aijia.ipcameraproject.utils.CustomBufferHead;
import com.aijia.ipcameraproject.utils.CustomVideoRecord;
import com.aijia.ipcameraproject.utils.HttpUtils;
import com.aijia.ipcameraproject.utils.MyRender;
import com.aijia.ipcameraproject.utils.SpeckUtil;
import com.aijia.ipcameraproject.utils.SystemValue;
...
public class PlayActivity extends Activity implements OnTouchListener,OnGestureListener, OnClickListener, PlayInterface ,CustomAudioRecorder.AudioRecordResult
{
	...
	private ImageButton btn_foucs,ptzAudio,ptztalk,ptzDefaultSet,ptzBrightness,ptzContrast,ptzTake_photos,ptzTake_vodeo,ptzResolutoin,preset;
...
//脑电波
   ...
   private  boolean isStartBrainLink=false;
	//百度语音合成
	private int i=0;//拍照张数标志
	private SpeckUtil speckUtil;
	...
	protected void onResume() {
		super.onResume();
		speckUtil = new SpeckUtil();
		speckUtil.initSpeack(this);
	}
	...
	private boolean isTime=false;//是否开始10S计数
	@Subscribe(threadMode = ThreadMode.MAIN)
	public void acceptMsg(MessageEvent messageEvent) {
		Toast.makeText(this,"1",Toast.LENGTH_SHORT).show();
		speckUtil.speakMsg("请在10秒内集中注意力。");
//		isTakepic=true;
		isStartBrainLink=true;
		if(!isTime){
			isTime=true;
			Timer timer=new Timer();//10秒后关闭脑电波
			timer.schedule(new TimerTask() {
				@Override
				public void run() {
					isStartBrainLink=false;
					isTime=false;
					speckUtil.speakMsg("注意力不足，无法验证！");
				}
			},10000);
		}
	}
	...
	@Subscribe(threadMode = ThreadMode.MAIN)
	public void acceptAttention(AttentionEvent attention) {
	     if(isStartBrainLink){
			 if(attention.getValue()>70){
				 speckUtil.speakMsg("正在拍照，请注意看摄像头！");
				 isTakepic=true;
				 isStartBrainLink=false;
				 isTime=false;
			 }
	...
	private void findView() {
		...
		btn_foucs= (ImageButton) findViewById(R.id.btn_focus);
		...
		btn_foucs.setOnClickListener(this);
	...
	public void onClick(View v) {
		...
		case R.id.ptz_vert_tour:
			...
			break;
			case R.id.btn_focus://调整焦距
				if (isBig) {
					isBig = false;
					NativeCaller.PPPPCameraControl(strDID,18,0);
				} else {
					isBig = true;
					NativeCaller.PPPPCameraControl(strDID,18,1);
				}
				break;
   ...
   //判断sd卡是否存在
	...
	private synchronized void savePicToSDcard(final Bitmap bmp) {
		...
		try {
			...
			++i;
			Log.e("", i+"");
			final File file = new File(div, strDate + "_"+ strDID + "_"+ i +".jpg");
			...
			if (bmp.compress(Bitmap.CompressFormat.JPEG, 70, fos))
			{
				...
				runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						Toast.makeText(PlayActivity.this,"拍照成功",Toast.LENGTH_SHORT).show();
						speckUtil.speakMsg("正在验证，请稍后！");
						if(file!=null){
							HttpUtils.sendVeriflyPictrue("/distinguish", file,"msg", new StringCallback() {
								@Override
								...
								public void onResponse(String response, int id) {
									...
									if(loginResponse.getData().equals("200")){
										Toast.makeText(PlayActivity.this,loginResponse.getMsg(),Toast.LENGTH_LONG).show();
										int index=loginResponse.getMsg().indexOf(",");
										speckUtil.speakMsg(loginResponse.getMsg().substring(0,index)+"欢迎光临艾佳天诚信息技术有限公司。");
									}else{
										Toast.makeText(PlayActivity.this,loginResponse.getMsg(),Toast.LENGTH_LONG).show();
										speckUtil.speakMsg("识别失败，请注意看摄像头");
									}
								}
							});
						}
